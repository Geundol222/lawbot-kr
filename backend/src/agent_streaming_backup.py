...
- 충분한 정보를 얻으면 종료하세요"""
        ...
        try:
            ...
            answer_generation_prompt = SystemMessage(
                content="""수집된 법령/판례 정보를 바탕으로 답변을 작성하세요.

**지침:**
- 도구에서 전달된 조문 내용과 판례 요지를 모두 반영하세요.
- 5인 미만 적용 여부처럼 논쟁/예외가 있는 경우, 단정하지 말고 법령(제11조/시행령 등)과 판례/행정 해석을 함께 제시하세요.
- 판례 요지가 있으면 별도 섹션으로 요약하고, 결론에서 판례 내용을 우선 반영하세요.
- 결론은 간결히, 근거(법령·판례)는 리스트로 명시하세요.
